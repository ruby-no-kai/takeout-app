...
export const TrackPage: React.FC<Props> = () => {
  ...
  if (!conferenceData) {
    return <>
      {conferenceError ? <Box my={2}><ErrorAlert error={conferenceError} /></Box> : null}
      <p>Loading</p>
    </>;
  }
  ...
  const tracks = conference.track_order.filter((k) => conference.tracks.hasOwnProperty(k)).map((k) => conference.tracks[k]);
  ...
  return <>
      <Tabs isLazy index={trackIndex} onChange={onTabChange}>
        <TabList>
          {tracks.map((t) => <Tab key={t.slug}>{t.name}</Tab>)}
        </TabList>
        <TabPanels>
          {tracks.map((t) => {
            return <TabPanel key={t.slug}>
              <TrackView track={t} />
            </TabPanel>
          })}
        </TabPanels>
